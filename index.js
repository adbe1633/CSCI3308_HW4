...
$(document).ready(function() {
    ...
    Object.keys(state_info).forEach(function(state) {
        ...
        var url =
            'https://api.forecast.io/forecast/a721d9b15e115c08cb34d2184cc025b7/' + latitude + ',' + longitude;
        $.ajax({url:url, dataType:"jsonp"}).then(function(data) {

            if (data.currently.temperature <= 10){
                $(`#${state}`).css('fill', 'blue');
            }
            else if(data.currently.temperature > 10 && data.currently.temperature <= 30){
                $(`#${state}`).css('fill', 'cyan');
            }
            else if(data.currently.temperature > 30 && data.currently.temperature <= 50){
                $(`#${state}`).css('fill', 'green');
            }
            else if(data.currently.temperature > 50 && data.currently.temperature <= 80){
